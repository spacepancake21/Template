...
gulp.task('dist-dev', ['build-dev', 'images-dev', 'fonts-dev', 'extras-dev'], function () {
	return gulp.src('dist.dev/**/*').pipe(size({title: 'build'}));
});

// Сборка и вывод размера содержимого папки Prod
gulp.task('dist-prod', ['build-prod', 'images-prod', 'fonts-dev', 'extras-prod'], function () {
